...
func nodosPosiblesVecinosLatis(vertexindex int) []int {
    vecinos := make([]int, 0)
    vecinoUp := vertexindex-10
    if vecinoUp >= 0{
        vecinos = append(vecinos,vecinoUp)
    }
    vecinoDown := vertexindex+10
    if vecinoDown <= 99{
        vecinos = append(vecinos,vecinoDown)
    }
    if (vertexindex % 10) == 0 {
        vecinoRight := vertexindex+1
        vecinos = append(vecinos,vecinoRight)
        vecinoUpRight := (vertexindex-10)+1
        if vecinoUpRight >= 0{
            vecinos = append(vecinos,vecinoUpRight)
        }
        vecinoDownRight := (vertexindex+10)+1
        if vecinoDownRight <= 99{
            vecinos = append(vecinos,vecinoDownRight)
        }
    }else{

        if (vertexindex % 10) == 9 {
            vecinoLeft := vertexindex-1
            vecinos = append(vecinos,vecinoLeft)
            vecinoUpLeft := (vertexindex-10)-1
            if vecinoUpLeft >= 0{
                vecinos = append(vecinos,vecinoUpLeft)
            }
            vecinoDownLeft := (vertexindex+10)-1
            if vecinoDownLeft <= 99{
                vecinos = append(vecinos,vecinoDownLeft)
            }
...
func graficaPlanaSVG(filename string, vertices []Vertex, aristas []Edge, solucion []int){
    cadenaSVG := "<!DOCTYPE html>\n"
    cadenaSVG += "<html>\n"
    cadenaSVG += "<body>\n"
    cadenaSVG += "<svg width=\"1100\" height=\"1100\">\n"
    solsize := len(solucion)
    solsizeStr := strconv.Itoa(solsize)

    cadenaSVG += "<text x=\"500\" y=\"40\"  fill=\"black\">Solucion de tamaño "+solsizeStr+"</text>\n"
    for _, arista := range aristas{
        aris_v1 := arista.v1_index
        aris_v2 := arista.v2_index
        x1axis := 100 + (aris_v1 % 10)*100
        y1axis := 100 + (aris_v1 / 10)*100
        x1axisStr := strconv.Itoa(x1axis)
        y1axisStr := strconv.Itoa(y1axis)
        x2axis := 100 + (aris_v2 % 10)*100
        y2axis := 100 + (aris_v2 / 10)*100
        x2axisStr := strconv.Itoa(x2axis)
        y2axisStr := strconv.Itoa(y2axis)
        cadenaSVG += "<line x1=\""+x1axisStr+"\" y1=\""+y1axisStr+"\" x2=\""+x2axisStr+"\" y2=\""+y2axisStr+"\" style=\"stroke:rgb(255,0,0);stroke-width:2\"/>\n"
    }

    for _, vert := range vertices{
        index := vert.index
        indexStr := strconv.Itoa(index)
        xaxis := 100 + (index % 10)*100
        yaxis := 100 + (index / 10)*100
        xaxisStr := strconv.Itoa(xaxis)
        yaxisStr := strconv.Itoa(yaxis)
        verticeEnSolucion := false
        for _, indx := range solucion{
            if indx == index{
                verticeEnSolucion = true
            }
        }
        color := "yellow"
        if verticeEnSolucion{
            color = "black"
        }
        cadenaSVG += "<circle cx=\""+xaxisStr+"\" cy=\""+yaxisStr+"\" r=\"20\" stroke=\"green\" stroke-width=\"4\" fill=\""+color+"\" />\n"
        cadenaSVG += "<text x=\""+xaxisStr+"\" y=\""+yaxisStr+"\" text-anchor=\"middle\" stroke=\"#51c5cf\" stroke-width=\"2px\" dy=\".3em\">"+indexStr+"</text>\n"
    }
    cadenaSVG += "</svg>\n"
    cadenaSVG += "</body>\n"
    cadenaSVG += "</html>"
    fmt.Println(cadenaSVG)
    appendFile(filename, cadenaSVG)
}

func graficaToFile(filename string, vertices []Vertex, aristas []Edge){
    cadenaSVG := ""
    vertexNumber := len(vertices)
    vertexNumberStr := strconv.Itoa(vertexNumber)
    cadenaSVG += vertexNumberStr+"\n"
    for _, arista := range aristas{
        aris_v1 := arista.v1_index
        aris_v1Str := strconv.Itoa(aris_v1)
        aris_v2 := arista.v2_index
        aris_v2Str := strconv.Itoa(aris_v2)

        cadenaSVG += aris_v1Str+","+aris_v2Str+"\n"
    }
...
func main() {
///////////////////////////////////////////////////////////////////////////////////////COMENTAR PARA NO CICLOS
//contadorDeTodo := 0
//for contadorDeTodo < 50{
///////////////////////////////////////////////////////////////////////////////////////COMENTAR PARA NO CICLOS
    //Leemos el archivo
    ...
    //Vamo a genera aristas para la grafica de 100 vertices en cuadricula
///////////////////////////////////////////////////////////////////////////////////////COMENTAR PARA NO CICLOS
    //randomAristas := rand.New(rand.NewSource(int64(contadorDeTodo)))
///////////////////////////////////////////////////////////////////////////////////////COMENTAR PARA NO CICLOS
    randomAristas := rand.New(rand.NewSource(int64(5)))
    edgesG := make([]Edge, 0)
    count = 0;
    for count < numOfVertexes {
        fmt.Printf("Vertice %d \n",count)
        posiVecinos := nodosPosiblesVecinosLatis(count)
        for _, vecino := range posiVecinos{
            fmt.Printf("    %d ",vecino)
            estaEnAristas := false
            //Checamos si esta la aristas con los vertices count y vecino ya esta definida en las aristas
            for _, arista := range edgesG{
                aris_v1 := arista.v1_index
                aris_v2 := arista.v2_index
                if (count == aris_v1 && vecino == aris_v2) || (count == aris_v2 && vecino == aris_v1) {
                    estaEnAristas = true
                }
            }
            if estaEnAristas == false{
                randonNumTemp := randomAristas.Float64()
                if randonNumTemp <= .3 {
                    edgesG = append(edgesG, Edge{vecino,count,1}) 
                }
            } 
        }
        fmt.Printf("\n")
    ...
    numIteracionSeguidasParaParar := 500
...
///////////////////////////////////////////////////////////////////////////////////////COMENTAR PARA NO CICLOS
    //randomSeed := contadorDeTodo
///////////////////////////////////////////////////////////////////////////////////////COMENTAR PARA NO CICLOS
    randomSeed := 0
    randomSeedStr := strconv.Itoa(randomSeed)
    filename := "GraficaSeed"+randomSeedStr
...
////////////////////////ACTUALIZACION GLOBAL////////////////////////////
    ...
    graficaPlanaSVG(filename+".svg", vertexesG, edgesG, min_sol)
    graficaToFile(filename+".txt", vertexesG, edgesG)

///////////////////////////////////////////////////////////////////////////////////////COMENTAR PARA NO CICLOS
//contadorDeTodo = contadorDeTodo + 1
//}
///////////////////////////////////////////////////////////////////////////////////////COMENTAR PARA NO CICLOS
}
